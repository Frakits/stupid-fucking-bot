...
client.on(Events.MessageCreate, async message => {
	...
	if (!message.author.bot) {
		if (message.content.includes("clear data") && message.author.id == "464025143790927872") {
			db = {};
			message.reply("data has been resetted");
			return;
		}
		if (message.content.includes("show db") && message.author.id == "464025143790927872") {
			message.reply(JSON.stringify(db, null, 4));
			return;
		}
		if (message.content.includes("how much xp left")) {
			message.reply((Math.round(100 * (db[message.author.id].lv * 0.043) * 100)/100).toString());
			return;
		}
		let tsMultiplier;
